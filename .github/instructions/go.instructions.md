---
description: 'Go の慣用的な慣例とコミュニティ標準に従って Go コードを記述するための手順'
...
- Write comments in Japanese by default; translate only upon user request
...
- Write comments in complete sentences in Japanese by default
...
- For HTTP requests, do not reuse a consumed `req.Body`. Instead:
	...
	- Prefer configuring `req.GetBody` so the transport can recreate the body for redirects/retries: `req.GetBody = func() (io.NopCloser(bytes.NewReader(buf)), nil }`
...
- Write documentation in Japanese by default
